...
\pkg{gWidgets} API is intended to be a cross platform means
within an R session to interact with a graphics toolkit.  Currently,
there are four available toolkits.
\begin{itemize}
\item \pkg{gGWidgetsRGtk2} for \pkg{RGtk2} package which is the most
...
\item \pkg{gWidgetstcltk} for the \pkg{tcltk} package which is the
  ...
  or polished.
\item \pkg{gWidgetsWWW} is an implementation for the web using
  \pkg{Rpad} to provide web to R interactivity and javascript and
  the EXT JS JavaScript libraries to provide R to web
  interactivity. Unlike the other implementations, this package is
...
\pkg{gWidgetsrJava} or \pkg{gWidgetstcltk} are pointed out
...
The \code{gWidgets} API is still in the formative stages and likely will
...
\pkg{gWidgets} package is geared around these, and providing a
...
GUI and afterwards in programattically interacting with the GUI.  Each
widget constructor in \pkg{gWidgets} include the arguments
...
maintains a \texttt{gtk2} package to install a post-2.8 version of
...
There is also a packaged port of the GTK run time libraries availabel
through at \url{http://r.research.att.com/}. However, with this, the
\pkg{cairoDevice} package failed. This is the package that allows
\RFunc{ggraphics} to work in \pkg{gWidgetsRGtk2}.
...
[The \pkg{gWidgetsWWW} package is altogether different. That package
installs with the dependence \pkg{Rpad}, but must also be configured
to use a web server (perhaps Rpad's internal one) and find the extjs
libraries (\url{www.extjs.com}) which must be downloaded and placed in
a spot the web served can find.]




The \pkg{gWidgetstcltk} package is easier to install, as a
typical R installation has the \pkg{tcltk} package and libraries
already installed. The \pkg{gWidgetstcltk} package was designed
...
[For \pkg{gWidgetsrJava} use ``rJava''
...
A combo box list can also allow its value to be entered by typing,
<<>>=
...
perhaps be edited. [This isn't implemented in all toolkits. and in
...
with one of the \code{addHandler} functions. the handler below prints
...
container, but it isn't implementated, so a new window is created.]
...
The \pkg{gWidgetstcltk} pacakge is a little different from the
...
arugment, with a default using the left-hand side.
<<layout>>=
## now layout
...
the method \code{[<-}  for \code{glayout} containers is confusing, as
no \code{[} method exists due to the indexing refering to coordinates
and not indexes.)
...
within a handler. For instance, the \code{changeState} function could
...
function are reflected outside the scopeo of the function.
...
the list has a relatively simple structure. Each widget is specifed using

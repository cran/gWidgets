...
\begin{quotation}
  \textbf{NOTE:} This document does not (yet) reflect changes as of
  version 0.0-27 of gWidgets. The examples may or may not run without modification.
\end{quotation}
...
\RPackage{gWidgetsrJava} or \RPackage{gWidgetstcltk} are pointed out
in braces.] The GTK toolkit is interfaced via the \RPackage{RGtk2}
package of Michael Lawrence, which in turn is derived from Duncan
Temple Lang's \RPackage{RGtk} package. The excellent \RPackage{RGtk2}
package opens up the full power of the GTK2 toolkit, only a fraction
of which is available though \RPackage{gWidgetsRGtk2}.
...
\RPackage{gWidgets} package is geared around these, and providing a
familiar means to interact with the widgets once constructed. Each
widget constructor include arguments \RCode{container} to specify a
parent container to place it in; \RCode{handler}, to specify a handler
to respond to the main event that the widget has; and \RCode{action},
which is used to pass extra information along to any handler. Each
widget when constructed returns an object of S4 class which can be
manipulated using generic functions, some new and some familiar.
...
examples which show how to use \RPackage{gWidgets}. The man pages are
also a source information about the widgets, see \RCode{?gWidgets} for
...
\RCode{?gWidgets-dialogs}). The dialogs in \RCode{gWidgets} are
...
to make interacting with a widget as natural (to an R user) as
...
container has a \RFunc{names} method which refers to the tab labels, a
\RCode{[} method for referring to the widgets comprising the notebook
pages, and a \RFunc{length} method to return the number of notebook
pages.
...
give a handler when constructing the file chooser. The \RCode{file}
component of the handler argument gives the files name (not
\RCode{svalue(h\$obj)}, as \RCode{gfile} does not return an object to
call \RCode{svalue} on). For example, the function below is written to
...
(The first \RCode{svalue()} returns a character string, it is promoted
to an object using \RCode{get})
\begin{Scode}
...
\end{Scode}
[The \RCode{filter} argument is not available with \RPackage{gWidgetsrJava}]
...
the \RPackage{tcltk} package due to, I believe, Martin Maechler.
...
\RCode{gWidgets}. This example is a little lengthy, but hopefully
...
As an example of the \RCode{add} and \RCode{delete} methods of a
container, we show how one could create the \RCode{gexpandgroup}
widget, were it not implemented by the underlying toolkit
(eg. \RPackage{gWidgetstcltk}). This container involves an icon to
...
As an alternative to using the global \RCode{state}  variable, the
...
within a handler. For instance, the \RCode{changeState} function could
...
[In \RPackage{gWidgetsrJava} drag and drop is provided through Java
and not \RPackage{gWidgets}. One can drag from widget to widget, but
...
will be passed to the \RCode{dropdata} value of the handler for
\RCode{addDropTarget}. 
...
Alternatively, the notebook could be given as the container when the
widget is constructed, as in
\begin{Sinput}
  ggraphics(container = nb, label="plot")
\end{Sinput}
This is required to use notebooks in \RPackage{gWidgetstcltk}.
...
\RPackage{gWidgetsRGtk2}.] is used to present tree-like data. A
...
\RCode{TRUE} for a directory in this listing, and \RCode{FALSE} for a
...
mouse click. Implemented in \RPackage{gWidgets} are the functions
...
\begin{Scode}
  ...
  gfunction = function(f, window = gwindow(title=fName), ...) {
    ...
    ## Finalize the layout container in RGtk2
    visible(tbl) <- TRUE
